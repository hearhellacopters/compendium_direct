...
const Ailment_Data_Pars_Dif = (
    ...
    if (effect_id && effect_id.slidertype == "ranks" && effect_id.defaultrank != undefined) {
        rank = effect_id && effect_id[`rank${between_10(effect_id.defaultrank)}`]
    }
    //levels
    if (effect_id && effect_id.slidertype == "levels" && currentlevel != undefined) {
        rank = effect_id && effect_id[`rank${between_10(currentlevel)}`]
    }
    //turns
    if (effect_id && effect_id.slidertype == "turns" && currentturns != undefined) {
        rank = effect_id && effect_id[`rank${between_10(currentturns)}`]
    }
    //enemies
    if (effect_id && effect_id.slidertype == "enemies" && currentenemies != undefined) {
        rank = effect_id && effect_id[`rank${between_10(currentenemies)}`]
    }
    //stacks
    if (effect_id && effect_id.slidertype == "stacks" && currentstacks != undefined) {
        rank = effect_id && effect_id[`rank${between_10(currentstacks)}`]
    }
    //groupstacks
    if (effect_id && effect_id.slidertype == "groupstacks" && currentgroupstacks != undefined) {
        rank = effect_id && effect_id[`rank${between_10(currentgroupstacks)}`]
    }
    //debuffsrank
    ...
    if (effect_id && effect_id.slidertype == "buffsrank" && currentbuffsranks != undefined) {
        rank = effect_id && effect_id[`rank${between_10(currentbuffsranks)}`]
    }
    //fieldbuffsrank
    if (effect_id && effect_id.slidertype == "fieldbuffsrank" && currentfieldbuffsranks != undefined) {
        rank = effect_id && effect_id[`rank${between_10(currentfieldbuffsranks)}`]
    }
    //currenthp backwards
...
export default Ailment_Data_Pars_Dif