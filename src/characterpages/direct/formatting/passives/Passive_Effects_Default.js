...
const Passive_Effects_Default = ({
    ...
    if (require_ == "" && require__1 == "" && effect_ == "" && effect__1 == ""  && passive_ability.field == undefined) {
        return ("")
    } else {
        return (
            <div className={`${list == true ? "sliderbase enemyabilityinfobase wpadding" : "defaultlistholder"} ${base_color == undefined ? "defaultcondbase" : "Buffbase"}`}>
                ...
                {passive_ability.passive_cond_type == 3  && (effect_ != "" || effect__1 != "" || passive_ability.field != undefined)?
                    <div onClick={showmeraw}>
                        {add_formatting(`${require__1 == "" && require_ == "" ? "" : "\xa0┬ "}${require_}${require__1 != "" && require_ != "" ? " or " : ""}${require__1 != "" ? `${require__1}` : ""}`, "tl")}
                        {effect_display_pars(effect_, effect__1, require_, require__1, 1)}
                        {effect_ == "Field Effect" && passive_ability.hide_field != true ?
                            passive_ability.field && passive_ability.field.map((buffs, i) => (
                                <Ailment_Field_Attached
                ...
                {passive_ability.loc_tag != undefined ?
                    <div>
                        ...
                        {`\xa0┬ Requires:`}
                        <div>
                            {add_formatting(`\xa0└─ Active <${passive_ability.loc_tag}> [${ver == "GL" ? passive_ability.name : passive_ability.glname}] #${passive_ability.pa_id}`, "tl")}
                        </div>
                    </div>
                    : ""}
                {showraw == true ?
...
export default Passive_Effects_Default