...
import AilmentLevelFormatting from '../components/Buffs/AilmentLevelFormatting'
import { getQuery, getQueryStringVal, useQueryParam } from '../components/URLParams'
import reactStringReplace from "react-string-replace"

export default function AilmentLevelCondition ({
  ver,
  loc,
  file,
  newcompare,
  ...
  const rawData = Object.values(ailment_level_condition)

  const banerDisplayTerm = "level conditions";
  ...
  useEffect(() => {
    ...
    const searchit = makeUnique.filter((ailment) =>
      (`${ailment.trans} #${ailment.cond_id}`).toLowerCase().includes(searchTerm)
    );
    const getailmentfilter = searchit.filter(function (ef) {
      const newfilterpull = ef["require_id"] === condFilter;
      if (condFilter !== "") {
    ...
    <div>
      ...
      <div className="ultimaweaponitemholder">
        <div className="subtext">
          {addformatting(displayBanner)}
        </div>
        {cond_List.length > 0 ? (
          cond_List.map(cond_file => (
            <AilmentLevelFormatting
