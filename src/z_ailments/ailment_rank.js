...
import AilmentRankFormating from '../components/Buffs/AilmentRankFormating'
import { getQuery, getQueryStringVal, useQueryParam } from '../components/URLParams'

export default function AilmentRank ({
  ver,
  loc,
  file,
  newcompare,
  ...
  const rawData = Object.values(ailment_rank)

  const banerDisplayTerm = "ailment ranks";
  ...
  useEffect(() => {
    ...
    const makeUnique = filterholder
      ...
      .sort((a, b) =>
        reverse === false ?
          a.ailment_id - b.ailment_id :
          b.ailment_id - a.ailment_id);
    const searchit = makeUnique.filter((ailment) =>
      (`${ailment.name} ${ailment.jpname} #${ailment.ailment_id}`).toLowerCase().includes(searchTerm)
    );
    ...
    <div>
      ...
      <div className="event-search-reverse-holder">
        {showFilter == true ? "" :
          <ul className="bufftypes">
            <Link to={`/${ver}/ailments/ailment_rank/compare`} >
        ...
        {showFilter == true ? "" :
          <ul className="bufftypes">
            <Link to={`/${ver}/ailments/ailment_rank/new`} >
      ...
      <div className="filterholder noselect" id={showFilter ? "showfilteren" : "hiddenfilteren"}>
        ...
        <div className="margeholder">
          ...
          <ul className="bufftypes">
            <Link to={`/${ver}/ailments/ailment_rank/compare?filter=true`} >
              <Tippy content={`Added from update`}>
                <li className={`${newcompare == "compare" ? "filteractive" : "filterinactive"} buffbutton newbutton`}></li>
              </Tippy>
            </Link>
            <Link to={`/${ver}/ailments/ailment_rank/new?filter=true`} >
              <Tippy content={`Full list`}>
                <li className={`${newcompare == "new" ? "filteractive" : "filterinactive"} buffbutton fullbutton`}></li>
              </Tippy>
            </Link>
      ...
      <div className="ultimaweaponitemholder">
        ...
        {listBuff.length > 0 ? (
          listBuff.map(buffs => (
            <AilmentRankFormating
              key={buffs.ailment_id}
              ailment_rank={buffs}
