...
export default function AilmentCast ({
  ver,
  loc,
  file,
  newcompare,
  ...
  const rawData = Object.values(ailment_cast)

  const dispatch = useDispatch();

  const banerDisplayTerm = "ailments casts";
  ...
  useEffect(() => {
    ...
    const searchit = makeUnique.filter((ailment) =>
      (`${ailment.casts && ailment.casts.map(self => self.name)} ${ailment.casts && ailment.casts.map(self => self.jpname)}- #${ailment.id}`).toLowerCase().includes(searchTerm)
    );
    ...
    <div>
      ...
      <div className="event-search-reverse-holder">
        {showFilter == true ? "" :
          <ul className="bufftypes">
            <Link to={`/${ver}/ailments/ailment_cast/compare`} >
        ...
        {showFilter == true ? "" :
          <ul className="bufftypes">
            <Link to={`/${ver}/ailments/ailment_cast/new`} >
      ...
      <div className="filterholder noselect" id={showFilter ? "showfilteren" : "hiddenfilteren"}>
        ...
        <div className="filterholderflair">
          <ul className="bufftypes">
            <Link to={`/${ver}/ailments/ailment_cast/compare?filter=true`} >
              <Tippy content={`Added from update`}>
                <li className={`${newcompare == "compare" ? "filteractive" : "filterinactive"} buffbutton newbutton`}></li>
              </Tippy>
            </Link>
            <Link to={`/${ver}/ailments/ailment_cast/new?filter=true`} >
              <Tippy content={`Full list`}>
                <li className={`${newcompare == "new" ? "filteractive" : "filterinactive"} buffbutton fullbutton`}></li>
              </Tippy>
            </Link>
      ...
      <div className="ultimaweaponitemholder">
        ...
        {listBuff.length > 0 ? (
          listBuff.map((buffs,i) => (
            <div key={i} className="blackbase infonameholder">
            Casts ID #{buffs.id}
            {buffs.casts && buffs.casts.map(self => (
