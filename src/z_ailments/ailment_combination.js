...
import AilmentCombinationFormatting from '../components/Buffs/AilmentCombinationFormatting';
import { getQuery, getQueryStringVal, useQueryParam } from '../components/URLParams'

export default function Ailment_Combination ({
  ver,
  loc,
  file,
  newcompare,
  ...
  const rawData = Object.values(ailment_combination)

  const banerDisplayTerm = "ailments combinations";
  ...
  useEffect(() => {
    ...
    const searchit = makeUnique.filter((ailment) =>
      (`${AilmentNames[ailment.ailment_id] && AilmentNames[ailment.ailment_id].name} ${AilmentNames[ailment.ailment_id] && AilmentNames[ailment.ailment_id].jpname}`).toLowerCase().includes(searchTerm)
    );
