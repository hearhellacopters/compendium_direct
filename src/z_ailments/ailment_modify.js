...
import AilmentModifyFormatting from '../components/Buffs/AilmentModifyFormatting';
import { getQuery, getQueryStringVal, useQueryParam } from '../components/URLParams'

export default function AilmentModify ({
  ver,
  loc,
  file,
  newcompare,
  ...
  const rawData = Object.values(ailment_modify)

  const banerDisplayTerm = "ailments modifications";
  ...
  const loadMoreButton = () => {
    ...
    const newlistdisplay = listDisplay.concat(
      searchResults.limits.slice(newlimits)
    );
    setLimits(newlimits);
  ...
  useEffect(() => {
    ...
    const searchit = makeUnique.filter((ailment) =>
      (`${ailment.name} ${ailment.jpname}`).toLowerCase().includes(searchTerm)
    );
