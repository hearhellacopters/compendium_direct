...
import EnemyAbilitiesListingFormatting from './formatting/EnemyAbilitiesListingFormatting'
...
const Passives = ({ ProcessedEnemyAbilities }) => {
   ...
   useEffect(() => {
       ...
       if(Typesearch != null){
        ...
        if(filteredtype.length != 0){
          ...
          setCondFilter(filteredtype[0].battle_id)
        } else{
          setTypesearch("")
          setCondFilter("")
        }
  ...
  useEffect(() => {
    ...
    if (filterholder.length === 0) {
      ...
      const getcondfilter = searchit.filter(function (ef) {
        const newfilterpull = ef["battle_id"] === condFilter;
        if(condFilter !== "") {
          return newfilterpull;
        } else {
          return ef
  ...
  function onlyUnique(value, index, self) {
    ...
    value: typeListUnique.battle_id,
          ...
          <meta property="og:url" content="https://dissidiacompendium.com/search/abilities"/>
            ...
            <div className="content">
              ...
              listabilities.map(abilities =>(
                  <EnemyAbilitiesListingFormatting key={abilities.data_id} match={abilities}/>
                ))) : (
                  <div>No results</div>
                )}
