                            ...
                            <div key={self.AIKey} className='upgradedorg2'>
                                {(debug == true || showmeraw == true) ?
                                    <Tippy content={self.AIKey}>
                                        <div className='abilitydisplayfex'>{replacer_enemydesc(`\xA0┬ ${self.Chance1 < 100 || self.Chance1 > 100 ? `RNG chance of ≤ ${self.Chance1} to be used when` : "Used when"}:`)}</div>
                                    </Tippy>
                                    :
                                    <div className='abilitydisplayfex'>{replacer_enemydesc(`\xA0┬ ${self.Chance1 < 100 || self.Chance1 > 100 ? `RNG chance of ≤ ${self.Chance1} to be used when` : "Used when"}:`)}</div>
                                }
                                {self.labels.map((labels, i) => {
                                    ...
                                    return (
                                        labels.AbilityNum != undefined ?
                                            <div key={i}>{replacer_title(`${lastkey == i ? "\xA0└─ " : "\xA0├─ "}${addabilitynum(labels.label, labels.AbilityNum)}`)}</div> :
                                            <div key={i}>{replacer_title(`${lastkey == i ? "\xA0└─ " : "\xA0├─ "}${labels.label}`)}</div>
                                    )
                                })}
                    ...
                    {type2.map(self => (
                        self.labels != undefined ?
                            <div key={self.AIKey} className='Buffupgradedorg'>
                                {debug == true ?
                                    <Tippy content={self.AIKey}>
                                        <div className='abilitydisplayfex'>{replacer_enemydesc(`\xA0┬ ${self.Chance1 < 100 || self.Chance1 > 100 ? `RNG chance of ≤ ${self.Chance1} to trigger on` : "Triggers on"} ${self.target && self.target.target != undefined ? self.target.target : ""}${self.target && self.target.chance != undefined ? ` (with a ${self.target.chance}% chance)` : ""} when:`)}</div>
                                    </Tippy>
                                    :
                                    <div className='abilitydisplayfex'>{replacer_enemydesc(`\xA0┬ ${self.Chance1 < 100 || self.Chance1 > 100 ? `RNG chance of ≤ ${self.Chance1} to trigger on` : "Triggers on"} ${self.target && self.target.target != undefined ? self.target.target : ""}${self.target && self.target.chance != undefined ? ` (with a ${self.target.chance}% chance)` : ""} when:`)}</div>
                                }
