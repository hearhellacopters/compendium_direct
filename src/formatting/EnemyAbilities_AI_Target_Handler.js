...
import replacer_enemydesc from '../processing/replacer_enemydesc'
import Tippy from './TippyDefaults.js';

const AI_Target_Handler = ({
    ai,
    abilitylist,
    debug
}) => {
                ...
                {ai.map(self => (
                    self.labels != undefined ?
                        <div key={self.AIKey} className={change_color(self)}>
                            {debug == true ?
                                <Tippy content={self.AIKey}>
                                    <div className='abilitydisplayfex'>{replacer_enemydesc(`\xA0┬ ${self.Chance1 < 100 || self.Chance1 > 100 ? `RNG chance of ≤ ${self.Chance1} to target` : "Targets"} ${self.target && self.target.target != undefined ? self.target.target : ""}${self.target && self.target.chance != undefined ? ` (with a ${self.target.chance}% chance)` : ""} when:`)}</div>
                                </Tippy>
                                :
                                <div className='abilitydisplayfex'>{replacer_enemydesc(`\xA0┬ ${self.Chance1 < 100 || self.Chance1 > 100 ? `RNG chance of ≤ ${self.Chance1} to target` : "Targets"} ${self.target && self.target.target != undefined ? self.target.target : ""}${self.target && self.target.chance != undefined ? ` (with a ${self.target.chance}% chance)` : ""} when:`)}</div>
                            }
                            {self.labels.map((labels, i) => {
                                ...
                                return (
                                    labels.AbilityNum != undefined ?
                                        <div key={i}>{replacer_title(`${lastkey2 == i ? "\xA0└─ " : "\xA0├─ "}${addabilitynum(labels.label, labels.AbilityNum)}`)}</div> :
                                        <div key={i}>{replacer_title(`${lastkey2 == i ? "\xA0└─ " : "\xA0├─ "}${labels.label}`)}</div>
                                )
                            })}
                            {self.restrict != undefined ?
                                <div className='lightblue'>{replacer_title(self.restrict)}</div>
                                : ""}
                        ...
                        <div key={self.AIKey} className={change_color(self)}>
                            {debug == true ?
                                <Tippy content={self.AIKey}>
                                    <div className='abilitydisplayfex'>{replacer_enemydesc(`\xA0┬ ${self.Chance1 < 100 || self.Chance1 > 100 ? `RNG chance of ≤ ${self.Chance1} to targets` : "Targets"} ${self.target && self.target.target != undefined ? self.target.target : ""}${self.target && self.target.chance != undefined ? ` (with a ${self.target.chance}% chance)` : ""} when:`)}</div>
                                </Tippy>
                                :
                                <div className='abilitydisplayfex'>{replacer_enemydesc(`\xA0┬ ${self.Chance1 < 100 || self.Chance1 > 100 ? `RNG chance of ≤ ${self.Chance1} to targets` : "Targets"} ${self.target && self.target.target != undefined ? self.target.target : ""}${self.target && self.target.chance != undefined ? ` (with a ${self.target.chance}% chance)` : ""} when:`)}</div>
                            }
...
export default AI_Target_Handler