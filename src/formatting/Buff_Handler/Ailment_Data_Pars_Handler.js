    ...
    useEffect(()=>{
        if(effect_id && effect_id.multiply == true){
            if(effect_id && effect_id.multiplyslider == "debuffsmuliply"){
                const holder = rank && rank.value1 * currentdebuffsmuliply
                setdisplayrank({value1: holder})
            }
            if(effect_id && effect_id.multiplyslider == "buffsmuliply"){
                const holder = rank && rank.value1 * currentbuffsmuliply
                setdisplayrank({value1: holder})
            }
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    if( effect_id == undefined){
        ...
        return (
            <div>
                ...
                <div className={`${currentlevel <= 0 && (effect_id.slidertype == "levels" || effect_id.defaultrank <= 0) ? "inactiveeffect" : ""}`}>
                {replacer(
