...
const EnemyDetailsDirect = ({ match, ProcessedCharacters, PartnerCharacters }) => {
    ...
    useEffect(()=>{
        ...
        if(activeOrb == true || activeForce == true){
            if(ran == false){
                var helpers_str = []
                if(match[`ForceRoleTag${ForcetimeTab}`] != undefined){
                    helpers_str = match[`ForceRoleTag${ForcetimeTab}`] && match[`ForceRoleTag${ForcetimeTab}`].split(" ")
    ...
    },[ran,activeForce,activeOrb,match,ForcetimeTab, PartnerCharacters,set_helpers,set_chars,sethelpers,setran,setcolumns])
                ...
                <Link className='click linktopage enemyiconholder makeleft enemytitledetails' to={`/bestiary/enemies/${match.battle_enemy_id}`}>
                    ...
                    {match.ElementsFlag != true ? "":
                        ...
                        <div className={`${selectedbuff.is_buff == 0 ? "Debuff" : "Buff"}base infobase makeright`}>
                            ...
                            :''}
                            {activeForce == true && getForce && getForce[`ForceGauge${1}`] != undefined ?
                            <ul className="enemybannertabs">
                                ...
                                {getForce[`ForceGauge${1}`] != undefined ?
                            ...
                            :""}
                            {activeForce == true && getForce != undefined && getForce[`ForceGauge${ForcetimeTab}`] != undefined ?
                            <div className={`enemyholderdesc${getForce && getForce.ForceGauge1 != undefined ? "2":""}`}>
                                <div className="enemyforcesubbanner">Force Gauge</div>
                                {getForce[`ForceGauge${ForcetimeTab}`].map((self,i)=>(
                            ...
                            <div className="subtext">{"*Doesn't take resistances into account"}</div>
                            </div>
                            :""}
                            {activeGraphic == true ?
                            ...
                            <div className="enemyholderdesc margtop lighterblue">
                                <div className="orbcondunit">
                                    ...
                                    getOrb.LufeniaCons.map(self=>(
                                        <div key={self} className="orbconbholder">
                                            <div className={self.slice(0, self.indexOf(":"))}>
                                               {addformatting(self)}
                                            </div>
                                        </div>
                                    ))   
