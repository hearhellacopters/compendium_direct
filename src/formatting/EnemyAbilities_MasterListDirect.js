...
import EnemyAbilityHandoffDirect from './EnemyAbilityHandoffDirect';
...
const EnemyAbilities_MasterListDirect = ({stats, ai,showmeraw, battle_enemy}) =>{
    ...
    enemyabilities && enemyabilities.map((self,i)=>{
        Object.assign(abilitydebug,{[i+10]:{
            AIAbilityNum: i+10,
            Name: self.Name,
    ...
    return(
        enemyabilities!= undefined && fullcommands != undefined && fullcommands.length != 0 ?
            ...
            :""}
            {showmeraw== true ?
            <ReactJson iconStyle={"square"} quotesOnKeys={false} name={"All AI"} displayDataTypes={false} collapsed={true} theme={"threezerotwofour"} src={enemyai}/>
            :""}
            </>
        :
...
export default EnemyAbilities_MasterListDirect