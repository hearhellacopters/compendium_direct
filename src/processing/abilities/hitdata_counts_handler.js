import hitdata_counter from './hitdata_counter.js'

export default function hitdata_counts_handler (hitfull){
    ...
    Object.keys(hitfull).forEach((key) => {
        if (key == "B1" ||
            key == "B2" ||
            key == "B3" ||
            key == "B4" ||
            ...
            Object.assign(new_hit_pars, { [key]: hitfull[key] })
        } else {
            if (hitfull[key].show != false) {
                hit_num = hit_num + 1
                Object.assign(new_hit_pars, { [`hit_${hit_num}`]: hitfull[key] })
            }
        }
    })
