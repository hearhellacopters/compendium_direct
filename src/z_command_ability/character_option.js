...
import OptionStandalone from '../components/Abilities/OptionStandalone.js';
...
function CharacterOption ({
  ...
  const rawData = Object.values(character_option)

  const banerDisplayTerm = "character options";
  ...
  useEffect(() => {
    ...
    const searchit = makeUnique.filter((ailment) =>
      (`${ailment.chara_id_ && char_id[ailment.chara_id_] && char_id[ailment.chara_id_].CharacterName} ${CommandNames[ailment.original_label_] && CommandNames[ailment.original_label_].name} ${CommandNames[ailment.change_label_] && CommandNames[ailment.change_label_].name} - #${ailment.data_id}`).toLowerCase().includes(searchTerm)
    );
  ...
  //type list
  const typeListArray2 = Object.values(master_index.charid).map((typeListUnique) => ({
...
export default trackWindowScroll(CharacterOption)