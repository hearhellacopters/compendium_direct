...
import CallCharAbilitiesHandoff from './CallCharAbilitiesHandOff';
import Loading from './_loading'
import DevSwitch from '../redux/DevSwitch';

const CallCharAbilities = () =>{
    ...
    const filtered = ProcessedCharacters && ProcessedCharacters.filter(function (el) { 
      ...
      if(filtered != undefined ){
        return (
            <CallCharAbilitiesHandoff match={match} filtered={filtered}/>
        )
      } else {
          return(
            <Loading/>
...
export default CallCharAbilities;