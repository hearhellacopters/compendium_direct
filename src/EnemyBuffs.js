...
import EnemyBuffsFormating from './formatting/BuffsForEnemy'
...
const Buffs = ({ProcessedEnemyBuffs}) => {
  ...
  useEffect(() => {
    if(debuffs == false && buffs == false){
        ...
        const searchit = makeUnique.filter((e) =>
          (`${e.Name && e.Name.replace(/(<)/gm,"").replace(/(<)/gm,"")} ${e.JPName} ${e.Desc == undefined ? "" : e.Desc.replace(/(<)/gm,"").replace(/(<)/gm,"")}`).toLowerCase().includes(searchTerm)
        );
