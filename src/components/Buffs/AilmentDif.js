...
import replacer_titles from '../ReplacerCharacter'
import makediff from '../../processing/makediff';

export default function AilmentDif ({
    master_index,
    ver_old,
    buff_old,
    ver_new,
    ...
    useEffect(() => {
        ...
        if (buff_new.max_level == -1 && buff_new.aarg2 != undefined) {
            ...
            Object.assign(buff_new,{max_level: buff_new.aarg2 })
        }
        if (buff_new.max_level == -1 && buff_new.aarg2 == undefined) {
            setHighestlvl_new(10)
            Object.assign(buff_new,{max_level: 10 })
        }
    ...
    useEffect(() => {
        ...
        if (buff_old.max_level == -1 && buff_old.aarg2 != undefined) {
            ...
            Object.assign(buff_old,{max_level: buff_old.aarg2 })
        }
        if (buff_old.max_level == -1 && buff_old.aarg2 == undefined) {
            setHighestlvl_old(10)
            Object.assign(buff_old,{max_level: 10 })
        }
        // eslint-disable-next-line 
    }, [buff_old])
    ...
    return (
        replacer_titles(makediff(Format_Cleaner(old_text).replace(/\s+$/g, ""), Format_Cleaner(new_text).replace(/\s+$/g, "")),forma)
    )
}